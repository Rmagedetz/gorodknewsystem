...
anket_data, balance, visits = st.tabs(['Анкета', 'Баланс', 'Посещаемость'])
...
with anket_data:
    ...
    def create_ds(df):
        ...
        cell = ws.cell(row=num_rows + 1, column=1,
                       value="Есть ли у ребенка заболевания (сердца, пищеварительной системы, "
        ...
        cell = ws.cell(row=num_rows + 1, column=1,
                       value="Есть ли у ребенка ограничения по физическим нагрузкам? Если да, "
                             "поясните подробнее.")
        cell.font = Font(name='TimesNewRoman', size=9)
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
        cell.border = border
        ...
        cell = ws.cell(row=num_rows, column=1,
                       value="даю свое согласие на получение информационных сообщений от детского "
                             "клуба Городок")
        cell.font = Font(name='TimesNewRoman', size=9)
        cell.alignment = Alignment(horizontal="center", vertical="bottom", wrap_text=True)
        ...
        cell = ws.cell(row=num_rows, column=1,
                       value="подтверждаю свое согласие на обработку своих персональных данных и"
                             "своего ребенка ")
        cell.font = Font(name='TimesNewRoman', size=9)
        cell.alignment = Alignment(horizontal="center", vertical="bottom", wrap_text=True)
...
with balance:
    rec_by_child = sql.Payments.get_df(child_name=child_selector)
    deb_by_child = sql.Debits.get_df(child_name=child_selector)
    from_childs_balance = sql.Payments_from_balance.get_df(child_name=child_selector)
    rec_total = rec_by_child['pay_sum'].sum()
    deb_total = deb_by_child['pay_sum'].sum()
    from_total = from_childs_balance['pay_sum'].sum()
    balance = rec_total - deb_total - from_total
    st.header(f'Текущий баланс: {balance}')
    receipts, debits, from_balance = st.tabs(['Приходы', 'Списания', 'Оплаты с баланса'])
    with receipts:
        recs = st.dataframe(rec_by_child,
                            column_config={
                                "id": "ID",
        ...
        st.subheader(f'Всего платежей на: {rec_total}')
    with debits:
        debs = st.dataframe(deb_by_child,
                            column_config={
                                "id": "ID",
                                "datetime": st.column_config.DatetimeColumn('Время',
                                                                            format='DD.MM.YY - HH:mm'),
                            ...
                            hide_index=True)
        st.subheader(f'Всего списаний на: {deb_total}')
        with from_balance:
            debits_df = sql.Payments_from_balance.get_df()
            ...
            def del_debit():
                ...
                if st.button('Удалить списание', key='delete_debit_accept'):
                    sql.Payments_from_balance.delete_record(id=debit_id)
                    st.rerun()


            from_table = st.dataframe(from_childs_balance,
                                      column_config={
                                          "id": "ID",
                                          "datetime": st.column_config.DatetimeColumn('Время',
                                                                                      format='DD.MM.YY - HH:mm'),
            ...
            st.subheader(f'Всего платежей на: {from_total}')
            if st.button('Удалить списание', key='delete_debit', width=170):
                del_debit()

with visits:
    visits_by_child = sql.Visits.get_df(child_name=child_selector)
    visits_by_child = visits_by_child[['season_name', 'filial_name', 'group_name', 'day_number', 'visit_status']]
    column_config = {
        'season_name': st.column_config.Column('Сезон',
                                               width='small'),
        'filial_name': st.column_config.Column('Филиал',
                                               width='small'),
        'group_name': st.column_config.Column('Группа',
                                              width='small'),
        'day_number': st.column_config.Column('День',
                                              width='small'),
        'visit_status': st.column_config.Column('Статус',
                                                width='small',
                                                help='Отметка о посещении')}

    show = st.dataframe(visits_by_child, column_config=column_config, hide_index=True)
